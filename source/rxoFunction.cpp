...
int getRecentTrack(int iOrder, int isDrumTrack){ //最近使ったトラックを返す
	...
	j=0;
 	if(isDrumTrack==0){
		for(i=0;i<MAXMELODY;i++){
			if((MAXMELODY-1) - iOrder == j)return iRecentTrackM[i];
			j++;
		}
	}else{
		for(i=0;i<MAXDRAM;i++){
			if((MAXDRAM-1) - iOrder == j)return iRecentTrackD[i];
			j++;
		}
	}
...
int ReverseTrackCode(char* strTrack)
{
	...
	char ch1 = strTrack[i];
	char ch2 = strTrack[i + 1];

	char upper1 = (ch1 >= 'a' && ch1 <= 'z') ? (ch1 - 'a' + 'A') : ch1;
	char upper2 = (ch2 >= 'a' && ch2 <= 'z') ? (ch2 - 'a' + 'A') : ch2;

	int hasExtension = (upper2 == 'X');

	if (upper1 >= '1' && upper1 <= '8') {
		int base = upper1 - '1';
		return hasExtension ? (base + 8) : base;
	}

	int alphaBase = -1;
	switch (upper1) {
	case 'Q': alphaBase = 16; break;
	case 'W': alphaBase = 17; break;
	case 'E': alphaBase = 18; break;
	case 'R': alphaBase = 19; break;
	case 'T': alphaBase = 20; break;
	case 'Y': alphaBase = 21; break;
	case 'U': alphaBase = 22; break;
	case 'I': alphaBase = 23; break;
	default:  return 99;
	}

	return hasExtension ? (alphaBase + 8) : alphaBase;
}
